...
def max_filter_activation(model, layers_to_display, selected_filters = None,
                          plot_size=(3,3), filters_per_row=4, 
                          step=1, iter=100, row_by_row=False):
    ...
    for i in range(len(layers_to_display)):
        ...
        for j in f_range:
            ...
            subplot_num = j   
            if(row_by_row == True): 
                subplot_num = subplot_num % filters_per_row + 1
                
            plt.subplot(num_rows, filters_per_row, subplot_num)
            plt.imshow(deprocess_image(input_img_data))
            plt.axis('off')
        
            if(row_by_row and subplot_num == filters_per_row):
                plt.show()
